...
export default function TiptapEditor({ value, onChange }: TiptapEditorProps) {
  ...
  return (
    <div className="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-zinc-900/20">
      {/* Editor Toolbar */}
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm transition-all hover:bg-slate-200 dark:hover:bg-zinc-800 ${
            editor.isActive("bold")
              ? "bg-red-600 text-white dark:bg-orange-600 dark:text-white font-bold"
              : "text-slate-750 dark:text-zinc-300"
          }`}
          title="Bold"
        >
          B
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm transition-all hover:bg-slate-200 dark:hover:bg-zinc-800 ${
            editor.isActive("italic")
              ? "bg-red-600 text-white dark:bg-orange-600 dark:text-white italic font-bold"
              : "text-slate-750 dark:text-zinc-300"
          }`}
          title="Italic"
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm transition-all hover:bg-slate-200 dark:hover:bg-zinc-800 ${
            editor.isActive("heading", { level: 2 })
              ? "bg-red-600 text-white dark:bg-orange-600 dark:text-white font-bold"
              : "text-slate-750 dark:text-zinc-300 font-semibold"
          }`}
          title="Heading 2"
        >
          H2
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm transition-all hover:bg-slate-200 dark:hover:bg-zinc-800 ${
            editor.isActive("bulletList")
              ? "bg-red-600 text-white dark:bg-orange-600 dark:text-white font-bold"
              : "text-slate-755 dark:text-zinc-300"
          }`}
          title="Bullet List"
        >
          • List
